...
class Hand_Off(MujocoEnv, EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def __init__(self, reward_type: str = "dense", **kwargs):
        ...
        self.si_unit_hand_off_site = self._model_names.site_name2id["si_unit_hand_off_site"]
        self.si_unit_hand_off = self.data.site_xpos[self.si_unit_hand_off_site].ravel()
    ...
    def get_distance(self, si_unit_hand_off, si_unit):
        distance = torch.sum(torch.norm(torch.from_numpy(si_unit_hand_off - si_unit), p=2, dim=-1), dim=-1)
    ...
    def step(self, a):
        ...
        goal_distance = self.get_distance(self.si_unit_hand_off, self.si_unit)
        goal_achieved = True if goal_distance <= 0.031 else False
    ...
    def _get_obs(self):
        ...
        distance = self.get_distance(self.si_unit_hand_off, self.si_unit)
        if distance <= 0.031:
            si_unit_on_base = 1.0
        else:
            si_unit_on_base = -1.0
        ...
        return np.concatenate(
            [
                qpos,
                self.si_unit_hand_off - self.si_unit,
                [si_unit_on_base],
            ]
        )
...
environment_name = "Hand_Off-v1"
checkpoint_path = "/Users/eirinipanteli/PycharmProjects/robotic-hand-simulations/Simulations/Hand_Off/Checkpoints/round_4_PPO_checkpoint_seed_1.pth"
save_path = ""
# train(environment_name, save_path, episode_steps=300)
# continue_train(environment_name, checkpoint_path, save_path, current_best_reward=10.0, episode_steps=300, round=4)
test(environment_name, checkpoint_path, save_path, episode_steps=400, total_num_episodes=10)